...
| **Version** | `1.0.0` |
| **Status** | `active` |
| **Responsible Executor** | `pop-ml-arquiteto` |
...
category: conversion
agent: pop-ml-arquiteto
elicit: false
autonomous: true
description: "Convert an SOP between human-readable Markdown, YAML, and JSON formats while preserving semantic equivalence. Supports bidirectional conversion: human-to-machine and machine-to-human. Validates that no information is lost in translation."
```
...
FILES:
  - Converted SOP: outputs/gerador-pop/converted/{sop-name}.{target-extension}
  - Validation report: outputs/gerador-pop/converted/{sop-name}-conversion-report.md
...
outputs:
  primary:
    path: "outputs/gerador-pop/converted/{sop-name}.{md|yaml|json}"
    format: "human-md|yaml|json"
    description: "Converted SOP in target format"

  secondary:
    - path: "outputs/gerador-pop/converted/{sop-name}-conversion-report.md"
      format: markdown
