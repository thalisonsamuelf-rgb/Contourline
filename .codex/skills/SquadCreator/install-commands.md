...
|---|-----|-------------------|--------|
| 1 | **Claude Code** | `.claude/agents/{slashPrefix}/` | `.md` with YAML activation block |
...
IDE DETECTION RESULTS
  ...
  [detected] Claude Code  - .claude/agents/
...
detection_fallback:
  if_no_ides_detected:
    action: |
      ...
      Options:
      1. Install for Claude Code (create .claude/agents/)
...
directory_creation:
  claude_code:
    directories:
      ...
      - ".claude/agents/{slashPrefix}/agents/"
      - ".claude/agents/{slashPrefix}/tasks/"
...
format_conversion:
  ...
  claude_code:
    file_extension: ".md"
    output_dir: ".claude/agents/{slashPrefix}/agents/"
    template: |
...
task_installation:
  for_each_detected_ide:
    ...
    claude_code:
      ...
      target: ".claude/agents/{slashPrefix}/tasks/{task}.md"
...
```yaml
claude_code_format:
  location: ".claude/agents/{slashPrefix}/agents/{agent-id}.md"
  structure:
    - "# /{slashPrefix}:{agent-id} Command"
    - "ACTIVATION-NOTICE"
