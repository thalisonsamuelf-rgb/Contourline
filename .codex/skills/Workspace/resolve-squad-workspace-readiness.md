...
```bash
node squads/aiox-workspace/scripts/resolve-squad-workspace-readiness.cjs --squad={slug} [--business={slug}] [--product={slug}] [--app={id}] [--context-type={type}]
```
...
2. `copy` e `hormozi` usam contexto `product`.
3. O registry canônico de squads, contexts e product consumers vive em `squads/aiox-workspace/config.yaml`.
4. O resolver do COO delega aos resolvers canônicos já existentes; não duplica regras locais.
5. `draft` permite exploração, mas não output final/canônico.
6. `blocked` interrompe a execução até corrigir as fontes faltantes.
7. `not_applicable` proíbe tratar ausência de setup como erro.
