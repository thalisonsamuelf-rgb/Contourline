...
const squadRoots = [
  path.join(ROOT, 'squads/aiox-design'),
  path.join(ROOT, 'squads/design'),
];
...
function validateRefs(component, field, folder) {
  ...
  for (const ref of refs) {
    const exists = squadRoots.some((squadRoot) => {
      ...
      return fs.existsSync(full) || fs.existsSync(yaml) || fs.existsSync(yml);
    });
