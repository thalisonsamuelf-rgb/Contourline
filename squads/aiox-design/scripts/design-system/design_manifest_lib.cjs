...
function resolveDesignBase(rootDir) {
  const canonical = path.join(rootDir, 'squads/aiox-design');
  if (fs.existsSync(canonical)) {
    return canonical;
  }

  return path.join(rootDir, 'squads/design');
}

function getDesignPaths(rootDir) {
  const base = resolveDesignBase(rootDir);
  return {
    base,
