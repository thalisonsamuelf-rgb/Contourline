...
Note: Only agents are synced to IDE commands. Tasks, checklists, templates,
...
DEFAULT_CONFIG = {
    "active_ides": ["claude", "cursor", "windsurf", "gemini", "antigravity", "codex"],
    "squad_aliases": {},
    "sync_mappings": {
        "squad_agents": {
...
def get_destination_path(
    ...
    if ide == "claude":
        # Only agents are synced to IDE commands
        type_dir_map = {
            "agent": "agents",
        }
        dir_name = type_dir_map.get(component_type, component_type + "s")
        ...
        return project_root / ".codex" / "skills" / squad_alias / filename
...
Examples:
  ...
  %(prog)s squad legal --force --ide cursor
    ...
    parser.add_argument("type", choices=["agent", "squad"],
                        help="Component type to sync (only agents are synced to IDE commands)")
