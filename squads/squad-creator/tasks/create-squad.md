...
**Context-first entrypoint (mandatory when available):**
...
- Do not bypass the routed workflow with ad hoc directory/file creation.
...
[PHASE 4: INTEGRATION]
    ...
    -> Prepare COO handoff for workspace integration when required
    |
[PHASE 5: VALIDATION]
...
**Operating Rule:**
- `workflows_mapped >= 10 OR agents_needed >= 8` means the squad is too large for direct creation.
- In that case, direct full-squad creation is vetoed.
- The only valid path is `PRD -> epic -> state -> resume`.
...
- Se o squad novo precisar `controlled_runtime_consumer` ou `workspace_first`, isso so e permitido quando `squads/c-level/` existir e vira handoff formal para `COO` via squad `c-level`
...
Use this mode when the native scope gate fires and a valid PRD exists.

```yaml
epic_execution_mode:
  required_inputs:
    - "docs/projects/{domain}/prd.md"
    - "Epic list with story decomposition"

  runtime_bootstrap:
    init: "node squads/squad-creator-pro/scripts/squad-state-manager.cjs init {slug} --prd docs/projects/{domain}/prd.md --execution-model epic"
    start: "node squads/squad-creator-pro/scripts/squad-workflow-runner.cjs start {slug} --prd docs/projects/{domain}/prd.md --epic N"

  rules:
    - "Execute one epic per session entrypoint"
    - "Persist canonical runtime state in .aiox/squad-runtime/create-squad/{slug}/state.json"
    - "Emit epic story artifacts under docs/projects/{domain}/epics/"
    - "Use *status and *resume to continue interrupted epic execution"
```
...
### Step 4.3.5: Prepare COO Workspace Handoff
...
prepare_coo_handoff:
  ...
  if_c_level_missing:
    action: "VETO"
    reason: "controlled_runtime_consumer/workspace_first are consulting-only levels and require squad c-level present"
      - read_paths
      - write_paths
      - template_namespace
...
- [ ] Runtime state exists at `.aiox/squad-runtime/create-squad/{squad_name}/state.json` (blocking)
...
- [ ] Every created workflow passes `workflow-validator.js` in strict mode (`--strict --fail-on-warnings`)
...
| VETO-SQD-004 | Missing workspace integration level | `config.yaml` has no `workspace_integration.level` field | HALT at PHASE 3 validation step. Every squad must declare its workspace integration level (none/read_only/controlled_runtime_consumer/workspace_first). |
| VETO-SQD-005 | Direct workspace mutation by squad-creator | Workflow attempts to write business/domain integration directly into `workspace/` during squad creation | HALT. Generate COO handoff instead. |
| VETO-SQD-006 | Smoke test failure | All 3 smoke test scenarios fail (activation, help, basic_task) | Transition to `failed` state via `smoke_failed` trigger. Squad is not safe to register. |
