...
- [ ] If `workspace_integration.level in {controlled_runtime_consumer, workspace_first}`, integration execution is delegated to `COO`/`c-level` and not performed directly by `squad-creator`
- [ ] If `workspace_integration.level in {controlled_runtime_consumer, workspace_first}`, `squads/c-level/` exists in the repo
