...
} = require(path.join(ROOT, 'squads', 'c-level', 'scripts', 'resolve-squad-workspace-readiness.cjs'));
...
function listOperationTemplates() {
  const templateRoot = path.join(ROOT, 'workspace', '_templates', 'operations');
...
async function buildWorkspaceContext(inputArgs) {
  ...
  const companySources = {
    company_profile: describeSource(
      'company_profile',
      `workspace/businesses/${args.business}/company/company-profile.yaml`,
      true,
      args.includeRaw
    ),
    icp: describeSource(
      'company_icp',
      `workspace/businesses/${args.business}/company/icp.yaml`,
      true,
      args.includeRaw
    ),
    brand: describeSource(
      'company_brand',
      `workspace/businesses/${args.business}/company/brand.yaml`,
      true,
      args.includeRaw
    ),
    offerbook: describeSource(
      'company_offerbook',
      `workspace/businesses/${args.business}/company/offerbook.yaml`,
      false,
      args.includeRaw
    ),
  };
  ...
  const operationsSources = {
    team_structure: describeSource(
      ...
      `workspace/businesses/${args.business}/operations/team-structure.yaml`,
      true,
      args.includeRaw
    ),
    pricing_strategy: describeSource(
      'operations_pricing_strategy',
      `workspace/businesses/${args.business}/operations/pricing-strategy.yaml`,
      true,
      args.includeRaw
    ),
    kpi_scorecards: describeSource(
      'operations_kpi_scorecards',
      `workspace/businesses/${args.business}/operations/kpi-scorecards.yaml`,
      true,
      args.includeRaw
    ),
    funnel_vsl_reference: describeSource(
      'operations_funnel_vsl_reference',
      `workspace/businesses/${args.business}/operations/funnel-vsl-reference.yaml`,
      false,
      args.includeRaw
    ),
    commission_design: describeSource(
      'operations_commission_design',
      `workspace/businesses/${args.business}/operations/commission-design.yaml`,
      false,
      args.includeRaw
    ),
  };
  ...
  if (missingRequired.length > 0) {
    blockers.push('Required company/operations sources are missing.');
  }
