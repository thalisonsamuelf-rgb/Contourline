...
FILE: outputs/gerador-pop/ml/{process-name}-sop-v{version}.yaml  (or .json)
...
outputs:
  primary:
    path: "outputs/gerador-pop/ml/{process-name}-sop-v{version}.yaml"
    format: "yaml|json"
    description: "Complete machine-readable SOP with state machine, tool mappings, and error handling"

  secondary:
    - path: "outputs/gerador-pop/ml/{process-name}-state-diagram.md"
      format: markdown
      description: "Mermaid state diagram for visual representation"

    - path: "outputs/gerador-pop/ml/{process-name}-validation-report.md"
      format: markdown
