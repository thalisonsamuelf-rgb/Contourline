...
async function main() {
  ...
  assert(
    ...
    'gerador-pop should stay in portable docs mode without canonical readiness support',
    fallback
  );
  ...
  const workspaceReady = await resolveEnvironmentContract({
    business: 'aiox',
    projectionPath: DEFAULT_PROJECTION_PATH,
  });
  ...
  assert(
    ...
    'gerador-pop should enter full workspace mode when COO operations readiness is ready',
    workspaceReady
  );
  assert(
  ...
  try {
    ...
    assert(
      ...
      'Enterprise marker must not bypass missing readiness support for gerador-pop',
      enterpriseContract
    );
    assert(
  ...
  console.log('PASS: gerador-pop environment contract tests passed');
}

main().catch((error) => {
  console.error('ERROR: gerador-pop environment contract tests failed');
