...
const OUTPUT_ROOT = path.join(ROOT, 'outputs', 'gerador-pop', 'extractions');
...
const PACKAGES = [
  {
    ...
    standards: [
      ['SOP Factory extract-sop', 'squads/gerador-pop/tasks/extract-sop.md'],
...
function renderIndex(packages) {
  ...
  return [
    ...
    `Generated: ${TODAY}`,
    '',
    'Canonical publish root: `outputs/gerador-pop/extractions/`',
    '',
    renderTable(
...
function main() {
  ...
  for (const pkg of PACKAGES) {
    ...
    writeRelative(
      path.join('outputs', 'gerador-pop', 'extractions', `draft-${pkg.slug}-sop-v0.1.md`),
      `${renderHumanDraft(pkg, sourceText, confidenceSection, gaps)}\n`
    );
    writeRelative(
      path.join('outputs', 'gerador-pop', 'extractions', `draft-${pkg.slug}-sop-v0.1.yaml`),
      `${renderMlYaml(pkg, gaps)}\n`
    );
    writeRelative(
      path.join('outputs', 'gerador-pop', 'extractions', `gaps-${pkg.slug}-${TODAY}.md`),
      `${renderGapReport(pkg, gaps)}\n`
    );
    writeRelative(
      path.join('outputs', 'gerador-pop', 'extractions', `confidence-${pkg.slug}-${TODAY}.md`),
      `${renderConfidenceReport(pkg, confidenceSection, lowConfidenceItems)}\n`
    );
    writeRelative(
      path.join('outputs', 'gerador-pop', 'extractions', `review-items-${pkg.slug}-${TODAY}.md`),
      `${renderReviewSummary(pkg, gaps, lowConfidenceItems, assumptions)}\n`
    );
    writeRelative(
      path.join('outputs', 'gerador-pop', 'extractions', `teachback-${pkg.slug}-${TODAY}.md`),
      `${renderTeachbackPlan(pkg)}\n`
    );
  }

  writeRelative(
    path.join('outputs', 'gerador-pop', 'extractions', 'thiagoroas-vsl-package-index.md'),
    `${renderIndex(PACKAGES)}\n`
  );

  console.log(`PASS: normalized ${PACKAGES.length} Thiago Roas extraction packages into outputs/gerador-pop/extractions`);
}
