...
const WORKSPACE_CONFIG_REL = 'workspace/workspace.yaml';
...
function inferDocumentScope(businessRelativePath) {
  ...
  if (parts[0] === 'movement' && parts[1]) {
    return `movement/${parts[1]}`;
  }
...
function buildBusinessSummary(businessSlug) {
  const companyProfilePath = path.join(ROOT, 'workspace', 'businesses', businessSlug, 'company', 'company-profile.yaml');
  ...
  const companyProfile = readYaml(companyProfilePath);
  return {
    company_name:
...
async function buildBusinessSopContext(inputArgs = {}) {
  ...
  const businessRegistered = Boolean(workspaceConfig?.businesses?.[args.business]);
  ...
  if (!businessRegistered) {
    blockers.push(`Business "${args.business}" is not registered in workspace/workspace.yaml.`);
  }
