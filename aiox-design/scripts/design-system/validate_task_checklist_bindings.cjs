...
const TASKS_DIR = path.join(ROOT, 'squads/aiox-design/tasks');
const CHECKLISTS_DIR = path.join(ROOT, 'squads/aiox-design/checklists');
...
function main() {
  if (!fs.existsSync(TASKS_DIR)) fail('Missing squads/aiox-design/tasks directory');
  if (!fs.existsSync(CHECKLISTS_DIR)) fail('Missing squads/aiox-design/checklists directory');
  ...
  for (const taskFile of taskFiles) {
    ...
    const refs = Array.from(
      sectionBody.matchAll(/`squads\/(?:design|aiox-design)\/checklists\/([^`]+\.md)`/g)
    ).map((m) => m[1]);
