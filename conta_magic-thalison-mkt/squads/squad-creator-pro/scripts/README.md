...
> Inventory: 23 operational scripts, 22 test scripts, 2 cross-provider scripts
...
| 21 | `load-business-sops.cjs` | Node.js | tasks/create-from-sop.md | Deterministic loader for canonical YAML SOP packages in `workspace/businesses/{slug}` | ACTIVE |
| 22 | `wave-loop.cjs` | Node.js | squad-workflow-runner.cjs (delegation), direct CLI | External SSH-style fresh-session loop for `*create-squad --wave=N` | ACTIVE |
| 23 | `wave-loop.sh` | Bash | direct CLI / SSH launcher | Thin shell wrapper for `wave-loop.cjs` | ACTIVE |
...
| 18 | `test_load_business_sops.cjs` | load-business-sops.cjs | Node.js |
| 19 | `test_squad_runtime_pipeline_e2e.cjs` | squad runtime pipeline | Node.js |
| 20 | `test_validate_squad_runtime_state_e2e.cjs` | validate-squad runtime state | Node.js |
| 21 | `test_wave_loop_runtime_e2e.cjs` | wave-loop.cjs | Node.js |
| 22 | `test_wave_loop_runtime_unit.cjs` | wave-loop.cjs | Node.js |
