...
workspace_domain_check:
  read_files:
    - "workspace/config.yaml"          # Lista de dominios e providers
...
workspace_domain_check:
  read_files:
    - "workspace/config.yaml"          # Dominios e providers registrados
    - "workspace/businesses/"             # BUs e produtos existentes
...
- Se o squad novo precisar `controlled_runtime_consumer` ou `workspace_first`, isso so e permitido quando `squads/aiox-workspace/` existir e vira handoff formal para `COO` via squad `c-level`
...
prepare_coo_handoff:
  ...
  if_c_level_missing:
    action: "VETO"
    reason: "controlled_runtime_consumer/workspace_first are consulting-only levels and require squad aiox-workspace present"
      - read_paths
      - write_paths
      - template_namespace
...
publish_chief_activation:
  command_sync:
    ...
    input:
      ...
      name: "{squad_name}"
    must_produce:
      - ".claude/agents/{slash_prefix}/agents/{entry_agent}.md"
