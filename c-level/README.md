...
O Squad C-Level nasceu da necessidade de ter um "dono" responsável por estruturar toda a informação da empresa dentro do AIOX. Não é um único profissional — é um squad composto por executivos C-Level, porque o trabalho cruza visão estratégica, operação, tecnologia e inteligência artificial simultaneamente.
...
|-------|------|------|-------------------|
| **COO** (coo-orchestrator) | Operations Orchestrator | Orchestrator | Líder do squad, coordena todos os C-Levels, estrutura geral |
...
```
FASE 0: Pre-Flight
  → Bootstrap + validate-c-level-essentials + workspace-context + scaffold templates
...
O squad `c-level` opera em `workspace_first`.
...
1. `workspace/workspace.yaml`
2. `workspace/user.md`
3. `workspace/config.md`
4. `workspace/domains/c-level/entities.yaml`
5. `workspace/domains/c-level/workflows.yaml`
