...
const designResolver = require('../../aiox-design/scripts/design-system/resolve_business_design_system.cjs');
...
const WORKSPACE_CONFIG_PATH = 'workspace/workspace.yaml';
...
function getBusinessCapabilityStatus(workspaceConfig, businessSlug, capability) {
  if (!workspaceConfig || !workspaceConfig.businesses || !businessSlug) {
    return null;
  }
  ...
  return business.capabilities[capability].status || null;
}
...
function resolveProductConsumerReadiness(args, squadConfig, integrationMode) {
  ...
  return {
    ...
    app_id: null,
    delegated_resolver: 'squads/c-level/scripts/resolve-product-readiness.cjs',
...
function resolveMovementReadiness(args, squadConfig, integrationMode) {
  ...
  const businessExists = Boolean(workspaceConfig && workspaceConfig.businesses && workspaceConfig.businesses[args.business]);
  ...
  const requiredPaths = [
    ...
    },
    {
      ...
      path: `workspace/workspace.yaml#businesses.${args.business}`,
      exists: businessExists,
      required: true,
    },
    {
      id: 'movement_entities',
      path: 'workspace/domains/movement/entities.yaml',
      exists: fs.existsSync(path.join(ROOT, 'workspace/domains/movement/entities.yaml')),
      required: true,
    },
    {
      id: 'movement_workflows',
      path: 'workspace/domains/movement/workflows.yaml',
      exists: fs.existsSync(path.join(ROOT, 'workspace/domains/movement/workflows.yaml')),
      required: true,
    },
    {
      id: 'movement_scaffold',
      path: 'workspace/domains/movement/brands/_scaffold',
      exists: fs.existsSync(path.join(ROOT, 'workspace/domains/movement/brands/_scaffold')),
      required: true,
    },
    {
  ...
  if (!businessExists) {
    status = 'blocked';
    reason = 'Business is not registered in workspace/workspace.yaml.';
    ...
    status = 'blocked';
    reason = 'Movement capability is not configured in workspace/workspace.yaml.';
  }
...
function resolveBrandReadiness(args, squadConfig, integrationMode) {
  ...
  const requiredPaths = [
    ...
    {
      id: 'brand_entities',
      path: 'workspace/domains/brand/entities.yaml',
      exists: fs.existsSync(path.join(ROOT, 'workspace/domains/brand/entities.yaml')),
      required: true,
    },
    {
      id: 'brand_workflows',
      path: 'workspace/domains/brand/workflows.yaml',
      exists: fs.existsSync(path.join(ROOT, 'workspace/domains/brand/workflows.yaml')),
      required: true,
    },
    {
      id: 'brand_templates',
